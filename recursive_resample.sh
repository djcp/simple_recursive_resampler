...
LAME_OPTIONS=" -V5 -q2 -v --lowpass 17 -b32 -B192 "
...
for FILE in `find -type f | grep -iE '(flac|ogg|mp3)$'`; do
	...
	then
		...
		then
			...
			TRANSCODE_OGG_COMMAND="/usr/bin/oggdec -o - \"$MP3_ROOT$FILE\" | /usr/bin/lame ${ID3_INFO} ${LAME_OPTIONS} -r - \"$OUTPUT_FILE\""
	...
	then
		...
		then
			...
			TRANSCODE_MP3_COMMAND="/usr/bin/lame ${ID3_INFO} ${LAME_OPTIONS} \"${FILE}\" \"${OUTPUT_FILE}\""
	...
	then
		...
		then
			...
			TRANSCODE_FLAC_COMMAND="/usr/bin/flac -dc \"${FILE}\" | /usr/bin/lame ${ID3_INFO} ${LAME_OPTIONS} - \"${OUTPUT_FILE}\""
